...
void Game_Options_On_INITDIALOG(HWND window)
{
	...
	if (Session.Type == GAME_INTERNET) {
		NetTiming::ConnectionQuality const quality = NetTiming::Connection_Quality_For_Settings(Session.Network_Timing_Target());

		handle = GetDlgItem(window, IDC_CTRLWOL_CONNECTION);
		if (handle) {
			SetSliderRangeAndPos(handle, 0, 3, static_cast<int>(quality));
			EnableWindow(handle, FALSE);
		}

		handle = GetDlgItem(window, IDC_SCROLL_SPEED_LABEL);
		if (handle) {
			Static_SetText(handle, Fetch_String(Network_Quality_Text_ID(quality)));
		}
