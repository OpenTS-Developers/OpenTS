...
extern int const sosCODECDiffTable[89 * 16];
extern unsigned short const sosCODECStepTable[89];
...
inline void Decode_Table_16(unsigned char const * source, short * dest, int samples, int deststride, int32_t & predicted, unsigned short & index)
{
...
inline void Decode_General(SosChannel & channel, unsigned char const * source, unsigned char * dest, int samples, int sourcestride, int deststride, int bits)
{
	for (int i = 0; i < samples; i++) {
