...
A screen opens as the original dialogs did, sliding out from the middle behind the side bars with the dialog sound, and a document can turn that off for itself. Pressing a control sounds the same click, which the rules name, and a disabled control stays silent. The options menu is the first screen rebuilt on the kit, so it looks like the original again rather than like the plain layout it had.
