...
system, so a loose file or a mix entry overrides what ships. The pictures, the
dialog font and the style sheets are read again whenever a side mounts its
archives, so a side can carry its own. A screen whose document, style sheet or
font will not load names the file in the log and answers as though the player
