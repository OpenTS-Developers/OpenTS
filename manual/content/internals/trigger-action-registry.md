...
- Every peer in a network game runs the handler on the same frame with the same state, so a handler mutates shared simulation state only and never branches on `PlayerPtr` except to affect presentation. A trigger arms an autosave with `Autosave.Arm()`; `Autosave_Service` writes it after deferred deletion, using the timed-save slots and multiplayer save boundary.
