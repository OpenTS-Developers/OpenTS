...
Repair works from a reduced figure. A structure that hands something out has that removed first: the price of its [`FreeUnit`](/keys/freeunit/), and, on the structure the first entry of [`PadAircraft`](/keys/padaircraft/) docks at, the average price of the whole pad aircraft list, rounded down, unless [`SeparateAircraft=yes`](/keys/separateaircraft/). Production adds both back, so the deduction shows up in repair bills alone. The reduced figure is the dock structure's; an aircraft's own repair bill works from its full written price.
