...
The reveal happens once, on the frame a [`DropZoneAnim`](/keys/dropzoneanim/) animation is created, and always for the local player. A flare a computer house drops lights the ground for whoever is watching; a flare the player drops reveals nothing for anyone else.
