...
This is the scale a cell's radar colors reach at the top of the height range. A cell is interpolated between [`LowRadarBrightness`](/keys/lowradarbrightness/) and this one by its own height, so the gap between the two is how strongly high ground stands out from low.
