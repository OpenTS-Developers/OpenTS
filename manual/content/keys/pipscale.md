...
The row's *contents* change once the type sets [`Passengers`](/keys/passengers/) above zero. Such an object draws one pip per slot, colored by an infantry passenger's own [`Pip`](/keys/pip/), and green for anything else, instead of the scaled fraction above. A hold with more than five units of space fills the row and leaves the rest of its load undrawn. The scale is still what decides how many slots are drawn, so a transport set to `PipScale=Ammo` shows its cargo in a row sized by its ammunition.
