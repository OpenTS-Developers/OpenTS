...
The flag has a second effect on every object type, buildings included: an object destroyed with it set spews first-growth Tiberium into five cells around it — north-west, north, east, south and west. Its own cell takes none. The north-west corner is reached rather than the center because the list opens with the no-direction marker, which the step that reads it folds round to that corner instead of standing still.
