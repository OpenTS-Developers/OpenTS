...
The trigger record's fields are covered by the [AI triggers](/mapping/ai-triggers/) page. `Max=4` is what makes this TeamType reachable at all. `Priority=8` lets a raid team take a member off a team left at the default `7`. Condition type `3` fires while the enemy's power output sits below its drain. The comparison block is therefore unused, and the condition object is never counted.

## The pass in outline

A house raises teams on a countdown of its own. Each time that countdown reaches zero the house asks for one suggestion. A single AI trigger is drawn at random from those whose gates and condition currently hold. The draw is weighted by a record the trigger keeps of how its past teams fared. One team is then created from each TeamType the trigger names: the first, and the second when it has one.

Filling those teams is then the teams' own work, and the members they are still missing are ordered through the same production the house runs for everything else. A computer house orders structures, vehicles, infantry and aircraft together in one pass. A computer house whose credits have fallen below [`AIAlternateProductionCreditCutoff`](/keys/aialternateproductioncreditcutoff/) narrows to structures or units instead, and in a campaign game no house narrows at all.

## When the pass runs

The countdown starts at `175 * <the house's position in the house list> + TeamDelays`, so houses do not all run their pass on the same [frame](/glossary/#frame). It is reset to [`TeamDelays`](/keys/teamdelays/) alone after every pass. Every house runs the pass, computer or human. Whether it reaches the AI triggers depends on two further gates: a `1` to `100` roll must come out at or below the house's [`RatioAITriggerTeam`](/keys/ratioaitriggerteam/), and the house's AI-trigger switch must be on. When either fails the pass ends with no suggestion and the countdown restarts anyway; a failed roll does not raise some other kind of team instead.

The switch starts off for every house, and exactly four things turn it on or off. The table shows what each of the four does to it. Outside a campaign game the third row is the only one a computer house reaches on its own, which is what the caution below turns on.
...
Outside a campaign game the deploy path is the only one of the four that a computer house reaches on its own. A computer house that starts a skirmish or multiplayer session with its base already built, rather than with an MCV to deploy, therefore never springs an AI trigger. Since the pass is not restricted to computer houses, the trigger action switches a human house into it just as readily.
:::
...
Those two counts then select one of two branches. The table sets both out. Only the second branch ever deletes a team, and it passes defensive triggers over regardless of how few the house holds.
...
The tech level requirement is not taken from the record. It is the highest `TechLevel` among the members of the trigger's TaskForces, so a trigger demands exactly what its teams demand. One trap follows: a member whose `TechLevel` is `-1` pushes the requirement to `11`, which silently makes the trigger unspringable.

:::caution[Outside a campaign the owner is not read]
The owner test runs only in a campaign game. In a skirmish or multiplayer session every house with the switch on evaluates every enabled trigger, whatever name the record holds. The side field and the condition are the only restrictions left that can tie a trigger to one kind of house.
:::

:::caution[Outside a campaign every team member must be buildable]
The buildable test accepts a recruitable object the house already owns in place of one it can build, but only in a campaign game. Outside a campaign, one member the house cannot currently build rejects the trigger outright, whether the type is held back by prerequisites, tech level or ownership. A TeamType with no TaskForce at all is rejected everywhere.
:::

## Difficulty

The three difficulty flags on a trigger mean Easy, Medium and Hard as the player chose them. That reading holds for every house in a campaign game, and for computer houses outside one.

In a campaign game the flag tested is simply the one matching the scenario's difficulty. Outside a campaign it takes two steps. The test reads the house's own [difficulty slot](/systems/difficulty/#from-the-setting-to-a-slot), which for a computer house is already the inverse of the setting the player chose. It then asks for the flag at the opposite end again. The table traces one computer house through both steps. Its left and right columns agree, because the two inversions cancel and the flags mean what they are named.
...
None of those six keys has a built-in list. When no rules layer sets one it stays empty, and the difficulty slot is used to index it anyway, reading storage that was never allocated. Give each of them one entry per difficulty.
:::

:::caution[More than one human player shifts the slot]
Outside a campaign, a computer house whose slot is not already 0 drops one slot when the session holds more than one human player and [`CompEasyBonus=yes`](/keys/compeasybonus/). The list index and the difficulty flag that is tested both move with it. A Medium game with two human players therefore reads entry 0 of each list and tests each trigger's Hard flag.
:::
...
A house is restricted to defensive triggers, and a trigger that is not defensive is then rejected outright, while either of these holds:
...
A house's enemy is the house it holds the most anger toward, and [damage is what moves that record](/systems/base-attacked/#anger-and-the-declared-enemy). Outside a campaign a computer house left without an enemy also picks one on a countdown. Nothing does that in a campaign, so a campaign house has no enemy until combat gives it one. Until then only defensive triggers can pass at all.
...
Every trigger that survives the gates is added to a distribution with its current weight, and one entry is drawn from that distribution. The three weights in a trigger record are truncated to whole numbers as the record is parsed. The current weight is truncated again as it is added. A trigger whose weight has fallen below `1` is therefore added to the distribution but can never be drawn from it.

## The track record

Each trigger has a current weight held between a minimum and a maximum of its own, and two counters: the runs it has had and the runs that succeeded. The counters start at zero and all three weights at `1` unless the record supplies them.
...
The recording is keyed on the first TeamType and nothing else. It does not check which trigger raised the team, or which house owns it. Every trigger in the scenario that names that TeamType first is credited with the same outcome, and a team the scenario placed by hand moves those records too. A trigger that only ever appears as somebody's second team never has its own record moved at all.

## From suggestion to team

Before the suggestion is returned the house's teams are scanned once more. If any team it owns is reforming, or is not under way, and that team's TeamType appears anywhere in the suggestion, the whole suggestion is discarded, and the second TeamType goes with it.

Every TeamType that survives is marked as autocreated, permanently for the rest of the session. [`Autocreate=yes`](/keys/autocreate/) sets the same mark from the start, but nothing in the selection above reads it: it is a recruitment setting. Being suggested applies it whether the section asked for it or not.

One team is then created from each surviving TeamType, subject to a second count test that is not the same as the eligibility test above. In a campaign game it compares `Max` against the teams of that type alive anywhere in the scenario. Outside a campaign it compares `Max` against the teams this house owns, the same count the eligibility test already applied. A campaign house can therefore clear the eligibility test and still create nothing.
...
Each pass fills at most one place in each TaskForce slot that is still short. A recruited transport is the exception: it brings its passengers with it, which can fill several places at once.

Candidates are ranked by distance from the cell of the TeamType's [`Waypoint`](/keys/waypoint/), or from the team's own center when it has no waypoint. The nearest one that may join is taken. An object is considered at all under any of these:

- the team's group is `-2`;
- the object has the team's group;
- the TeamType is [`Recruiter=yes`](/keys/recruiter/).

A candidate outside the group is pushed 50 cells further away in the ranking before the nearest is chosen. The team's group is the TeamType's [`Group`](/keys/group/#scope-teamtype), which falls back to the TaskForce's [`Group`](/keys/group/#scope-taskforce) while the TeamType leaves its own at `-1`. Every member's group is overwritten with it as the member joins.
...
6. It is not an aircraft that has a weapon and has no ammunition.
7. Its type matches one of the TaskForce's member lines, and that line still has room.

Which recruitable state step 4 tests depends on the autocreate mark. An unmarked TeamType reads the object's ordinary team-recruitable state and ignores its autocreate-recruitable state; a marked one does the reverse. Both start set on every object. Joining a team overwrites the object's autocreate-recruitable state with the TeamType's [`AreTeamMembersRecruitable`](/keys/areteammembersrecruitable/), so a team declared `AreTeamMembersRecruitable=no` locks its members away from every autocreated team that comes after it.

A team left with no members dissolves at once when it has already reached full strength or been set in motion. Outside a campaign it also dissolves [`DissolveUnfilledTeamDelay`](/keys/dissolveunfilledteamdelay/) frames after it was created, whether or not anybody ever joined it. That is the ordinary end of a team whose TaskForce the house could not fill.

## Production demand

Structures are chosen by [the base planner](/systems/ai-base-building/). Vehicles, infantry and aircraft are each chosen by their own routine, and the three share one shape. The vehicle routine differs in one way: it first considers replacing a lost harvester, and it can spend its turn on that. A routine that already has a type selected and unbuilt does nothing further.
...
1. **Demand.** A team of this house contributes to the tally under either of these:
   ...
   - **All of:** it is not forced active (a mark only a reinforcement group holds), and it has neither reached full strength nor been set in motion.

   Each unfilled place in a contributing team's TaskForce for a type of the routine's own kind adds one to that type's demand. The creation frame of the earliest contributing team is kept per type.

2. **Supply.** Every object of the house that is recruitable takes one off its type's demand. An object is recruitable when it is out of limbo, on no team, on a mission that allows recruitment, and not held back by its own recruitable state.
...
The uniform draw is not restricted to the types in most demand. The candidate list is emptied only when a type whose demand exceeds every demand seen so far is reached. It therefore holds every candidate found since the last such type, including candidates with less demand. Which candidates those are depends on the order the types are registered in.
...
A TeamType's [`Prebuild`](/keys/prebuild/) and [`LooseRecruit`](/keys/looserecruit/) are read into flags no decision reads, and so is [its own `TechLevel`](/keys/techlevel/#scope-teamtype). A trigger's tech level requirement comes from its teams' TaskForce members instead. [`AutocreateTime`](/keys/autocreatetime/) in `[AI]` is read into the rules and reaches only a disabled block.
