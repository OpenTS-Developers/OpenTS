...
Winning a campaign mission does not return the player to the menus. The game plays out the mission's endings, decides where the campaign goes, tears the scenario down and loads the next one, all without leaving the running session. Five pieces of state are carried across that boundary by hand, because nothing else survives it. This page owns the sequence and the carried state. What each setting does on its own belongs to that setting's page.
...
A **campaign** is one entry in the `[Battles]` list of `battle.ini`, and it supplies three things the progression needs: the [`Scenario`](/keys/scenario/#scope-campaign) it begins at, the [`CD`](/keys/cd/) its files are expected on, and the [`FinalMovie`](/keys/finalmovie/) it closes with.

Where a campaign goes after its first mission is not in `battle.ini` at all. It is in `MAPSEL.INI`, or `MAPSEL01.INI` for Firestorm, which holds one section per HouseType listing that house's **stages** in order under the numeric keys `1` upward. Each value names a stage's own section. That section names the scenario the stage stands for in a `Scenario=` key, and lists the stages it may lead to under numeric keys of its own. A mission does not remember a filename; it remembers its **stage**, which is a position in that per-house list. A mission file never names that position. A fresh campaign starts with its stage at the first entry of the list, and an advance moves the position to the stage it settled on. The file's sections and assignments are documented on [MAPSEL.INI](/formats/mapsel-ini/).
...
Steps 7 and 10 sit either side of the load. The state is read out of the outgoing mission, but the terms on which it is handed over are read out of the incoming one, because by step 10 the new scenario file has already been read. Those terms are the share of money, the ceiling on it and whether the timer is inherited. A mission reachable by two different routes therefore carries the same share over from either.

:::caution[An allow-win tag holds the victory for good]
The blockage count goes up once for each such tag as the scenario is read. A tag holding a single trigger, which is the ordinary shape, does not bring it back down by firing. A campaign house holding one can therefore be flagged to win and never receive the win. [Allow win](/mapping/actions/taction-allowwin/) owns the count, why firing leaves it standing, and the disposal that does clear it.
:::

## What survives the boundary

Five things are copied out of the won mission, and four of them are put back into the next one. Everything else about the outgoing scenario is discarded with it. The table gives what each one is taken from and what the next mission does with it; the column to read is the second.
...
| Difficulty | The player's own difficulty slot | Not restored here. The new scenario [handicaps each house](/systems/difficulty/#when-a-house-is-re-handicapped) as it reads the house |
...
The terms of the hand-over are written in the incoming mission's own `[Basic]` block, beside the keys that name where the campaign goes next:

```ini title="mission INI"
[Basic]
CarryOverMoney=0.5   ; share of the credits the player still held when the mission was won
CarryOverCap=5000    ; ceiling on the money carried over
TimerInherit=yes     ; resume the mission timer in the next mission
```
...
The grant is not consumed. The block still holds the previous mission's ending balance afterwards. Every path that applies it does so afresh, so a mission replayed after a loss and a mission restarted from the menu are each granted the same money again. The same is true of the inherited timer.
