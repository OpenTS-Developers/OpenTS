...
Every player has a button with their name and a bar beside it. Your own bar stays full and green. Another player's bar measures the time since their last frame message. It is green for four seconds, yellow until eight, and red after that, and it shrinks to a stub at twenty. Those are measures of silence, not of how far the wait has run. The line beneath counts down that wait, forty seconds by default: 2400 frames. A [launch file](/formats/spawn-ini/#a-game-against-other-machines) changes it with `ReconnectTimeout`, which is counted in frames of sixty to the second. The list under the line explains the situation and offers the choices below.
